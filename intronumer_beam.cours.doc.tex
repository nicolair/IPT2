...
\begin{algorithm}[H]
 ...
 \Comment{recherche de la plus petite puissance $p$ de $b$  telle que $x < p$. Alors $b^{n-1} = p/b \leq x$.}
 $p \leftarrow 1$\;
 \Tq{$p \leq x$}{
     $p\leftarrow p*b$}
 $p \leftarrow p / b$\;
...
\lstinputlisting[firstline=12, lastline=23]{intronumer.py}
\end{frame}

\begin{frame}
 \frametitle{Par les restes: code Python}
\lstinputlisting[firstline=30, lastline=34]{intronumer.py}
\end{frame}
